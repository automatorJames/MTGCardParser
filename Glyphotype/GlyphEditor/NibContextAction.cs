...
public record NibContextAction
{
    public EditorPropertyNib Nib { get; }
    public ContextActionType ActionType { get; }
    public string Html { get; }
    public bool AddSectionBreak { get; }

    public NibContextAction(EditorPropertyNib nib, ContextActionType type)
    {
        Nib = nib;
        ActionType = type;
    ...
    public static List<NibContextAction> GetNibContextOptions(EditorPropertyNib nib)
    {
        List<ContextActionType> actionList = [];

        var xOfType = nib.XOfType;
        var proptions = nib.Proptions;
        ...
        return actionList
            .Select(x => new NibContextAction(nib, x))
            .ToList();
    }
}
