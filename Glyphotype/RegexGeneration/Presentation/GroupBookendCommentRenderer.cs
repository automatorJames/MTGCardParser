...
{
    readonly Func<RegexBrick, string, RegexSpanKind?, SmartSpan> _spanFactory;
    ...
    public GroupBookendCommentRenderer(Func<RegexBrick, string, RegexSpanKind?, SmartSpan> spanFactory, int maxCommentLength)
    {
    ...
    public List<SmartSpan> Render(RegexBrickGroupBookend brick)
    {
        ...
        var paddingSpan = _spanFactory(brick, paddingLine, RegexSpanKind.CommentGroupBorderWall);

        List<SmartSpan> spans = [_spanFactory(brick, layout.StartCorner.ToString(), RegexSpanKind.CommentGroupBorderWall)];
        spans.AddRange(layout.CommentBeforePadding ? [.. commentSpans, paddingSpan] : [paddingSpan, .. commentSpans]);
        spans.Add(_spanFactory(brick, layout.EndCorner.ToString(), RegexSpanKind.CommentGroupBorderWall));
    ...
    static (string Text, RegexSpanKind Kind)[] GetCommentSegments(RegexBrickGroupBookend brick) => brick switch
    {
        RegexBrickGroupOpen open => BuildSegments(
            (open.TypeLabel, RegexSpanKind.CommentGroupOpenHeaderText),
            (open.TypeDisambiguator, RegexSpanKind.CommentGroupOpenHeaderDisambiguator)),
        RegexBrickGroupClose close => BuildSegments(
            (close.NameText, RegexSpanKind.CommentGroupFooterText),
            (close.QuantifierReminder, RegexSpanKind.CommentGroupFooterQuantifierReminder)),
        _ => throw new ArgumentOutOfRangeException(nameof(brick), brick, $"Unhandled {nameof(RegexBrickGroupBookend)} subtype")
    };

    static (string, RegexSpanKind)[] BuildSegments(params (string Text, RegexSpanKind Kind)[] segments) =>
