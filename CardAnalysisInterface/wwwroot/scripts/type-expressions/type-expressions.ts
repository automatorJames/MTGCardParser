...
const originalRegexContent: Map<HTMLElement, string> = new Map();
 ...
 */
function highlightRegexSpan(preElement: HTMLPreElement, start: number, end: number): void {
 ...
 */
function clearRegexHighlight(preElement: HTMLPreElement): void {
    const codeElement = preElement.querySelector('code');
 ...
 */
export function initTypeExpressionHover(): void {
    originalRegexContent.clear();

    const container: HTMLElement | null = document.querySelector('.type-card-container');
    if (!container) return;
    ...
    container.addEventListener('mouseover', (event: MouseEvent) => {
        const target = (event.target as HTMLElement)?.closest<HTMLElement>(
            '.property-capture-card, .canonical-representation, .capture-variant'
        );

        if (!target) return;

        hoveredElement = target;
        const typeCard = hoveredElement.closest<HTMLElement>('.type-card');
...
(window as any).initTypeExpressionHover = initTypeExpressionHover;