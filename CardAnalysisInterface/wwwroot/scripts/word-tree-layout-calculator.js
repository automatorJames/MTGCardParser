...
const columnIndexMap = new WeakMap(); // Stores the 1-based column index of a node.
const fanDeltaMap = new WeakMap(); // Stores the extra horizontal length for fanning connectors.
/**
 ...
 */
export function computeColumnOffsetsAndAssignFan(rootNodes, anchorX, anchorY, config) {
    ...
    const recordFanStats = (columnIndex, upCount, downCount) => {
        ...
        if (upCount > currentStats.maxUp)
            currentStats.maxUp = upCount;
        if (downCount > currentStats.maxDown)
            currentStats.maxDown = downCount;
    ...
    const processChildren = (children, parent) => {
        ...
        const assignGroup = (nodeArray, kind) => {
            ...
            if (kind === 'up')
                recordFanStats(columnIndex, nodeCount, 0);
            else
                recordFanStats(columnIndex, 0, nodeCount);
